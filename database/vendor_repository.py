...
def update_vendor_profile_db(cursor, user_id, company_name, vendor_name, owner_name, phone_number, gst_number, address, city, state):
    cursor.execute("""
        UPDATE vendors
        SET company_name = ?,
            vendor_name = ?,
            ...
            state = ?
        WHERE user_id = ?
    """, (company_name, vendor_name, owner_name, phone_number, gst_number, address, city, state, user_id))

...
def build_payment_report_blocks(payment_metrics, top_methods_df=pd.DataFrame(), is_status=False):
    metrics = [
        ("Total Revenue", money(payment_metrics.get("total_revenue", 0))),
        ("Total Transactions", f"{payment_metrics.get('total_transactions', 0)}"),
    ...
    if not top_methods_df.empty:
        tables.append({
            "title": "Transactions by Status" if is_status else "Methods by Volume",
            "df": top_methods_df,

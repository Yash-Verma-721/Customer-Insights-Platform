...
def show_vendor_analytics():
    render_header("Company Analytics", "Monitor company performance, fulfillment metrics, and profitability.", "Company Analytics")
    ...
    if df is None or df.empty:
        render_empty_state()
        render_footer("Company Analytics")
        return
    with branded_spinner("Loading company analytics..."):
    ...
    if not has_vendor:
        st.info("Awaiting company identification fields in the active dataset.")
    ...
    k1.metric("Companies", f"{metrics.get('total_vendors', 0):,}")
    ...
    if has_vendor and not profile.empty:
        ...
        with c1:
            ...
            fig_top = px.bar(top_vendors, x="vendor", y="revenue", title="Top Companies by Revenue", text_auto=".2s")
            ...
            fig_pie = px.pie(pie_df, names="vendor", values="revenue", title="Revenue Share by Company")
            st.plotly_chart(_apply_chart_layout(fig_pie, height=350), use_container_width=True)
            
        with c2:
            if not profile["avg_rating"].isna().all():
                fig_rate = px.histogram(profile, x="avg_rating", title="Company Rating Distribution", nbins=20)
        ...
        st.info("Company performance charts will be populated when company data is detected.")
    ...
    if has_vendor and not profile.empty:
        ...
        st.success(f"**Best Company:** {best_vendor['vendor']} is driving the most revenue ({money(best_vendor['revenue'])}).")
        st.warning(f"**Lowest Company:** {lowest_vendor['vendor']} generated the lowest revenue ({money(lowest_vendor['revenue'])}).")
        
        top_3 = profile.sort_values("revenue", ascending=False).head(3)
        leaders = ", ".join(top_3["vendor"].astype(str).tolist())
        st.info(f"**Revenue Leaders:** The top performing companies are {leaders}.")
    else:
        st.info("Company analytics provide visibility into the supply chain and help isolate underperforming businesses.")
    ...
    with st.expander("Company Summary"):
        if has_vendor and not profile.empty:
            ...
            display_df = display_df.rename(columns={"vendor": "company"})
    ...
    if st.button("Generate Company Report", type="primary"):
        with branded_spinner("Generating Company Report..."):
            ...
            st.session_state["vendor_report_bytes"] = generate_excel_bytes(blocks, "Company Report")

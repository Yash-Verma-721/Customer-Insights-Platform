...
def show_admin_settlements():
    ...
    with c_search:
        search_q = st.text_input("Search by Company or Order Code")
    with c_filter:
    ...
    for idx, row in filtered_df.iterrows():
        with st.expander(f"Company: {row['vendor_name']} | Order: {row['order_code']} ({row['settlement_status']})"):
            st.markdown(f"""
            **Product:** {row['product_name']}
            
            **Gross Amount:** ${row['gross_amount']:.2f}
            **Marketplace Commission ({row['commission_rate']}%):** ${row['commission_amount']:.2f}
            **Net Payout to Company:** ${row['net_amount']:.2f}

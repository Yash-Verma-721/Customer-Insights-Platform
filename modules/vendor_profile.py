...
def show_vendor_profile():
    ...
    with st.form("vendor_profile_form"):
        ...
        with col1:
            ...
            vendor_name = st.text_input("Vendor Name (Representative)", value=data.get("vendor_name", ""))
            phone_number = st.text_input("Phone Number", value=data.get("phone_number", "") or "")
        with col2:
            owner_name = st.text_input("Owner Name (Deprecated, use Vendor Name)", value=data.get("owner_name", ""))
    ...
    if submitted:
        if not all([company_name, vendor_name, phone_number, gst_number, address, city, state]):
            st.error("All core fields except Owner Name are required.")
        else:

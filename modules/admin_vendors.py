...
def show_admin_vendors():
    ...
    with c_search:
        search_q = st.text_input("Search by Company Name or Email")
    with c_filter:
        status_filter = st.multiselect("Filter by Status", ["Pending", "Approved", "Rejected", "Suspended"], default=["Pending", "Approved", "Suspended"])
        
    filtered_df = df.copy()
    if search_q:
        filtered_df = filtered_df[filtered_df['company_name'].str.contains(search_q, case=False, na=False) | 
    ...
    for idx, row in filtered_df.iterrows():
        with st.expander(f"{row['company_name']} ({row['vendor_status']}) - {row['user_email']}"):
            st.markdown(f"""
            **Vendor Representative:** {row['vendor_name'] or 'N/A'} | **Phone:** {row['phone_number'] or 'N/A'} | **GST:** {row['gst_number'] or 'N/A'}

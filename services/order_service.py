...
def change_order_item_status(user_id, order_item_id, old_status, new_status):
    ...
    try:
        ...
        if old_status == VendorOrderStatus.PACKED and new_status == VendorOrderStatus.SHIPPED:
            cursor.execute("SELECT product_id, quantity FROM order_items WHERE id = ?", (order_item_id,))
            oi_row = cursor.fetchone()
            if oi_row:
                p_id, qty = oi_row
                cursor.execute("UPDATE inventory SET current_stock = current_stock - ?, updated_at = ? WHERE product_id = ?", 
                               (qty, updated_at, p_id))

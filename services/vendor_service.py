...
def update_vendor_profile(user_id, company_name, vendor_name, owner_name, phone_number, gst_number, address, city, state):
    """Update vendor profile details for a specific user ID."""
    conn = get_connection()
    cursor = conn.cursor()
    try:
        update_vendor_profile_db(cursor, user_id, company_name, vendor_name, owner_name, phone_number, gst_number, address, city, state)
        conn.commit()
